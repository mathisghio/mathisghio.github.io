...
export function Waves({
  ...
  const movePoints = (time: number) => {
    ...
    lines.forEach((points) => {
      points.forEach((p: Point) => {
        ...
        const d = Math.hypot(dx, dy), l = Math.max(175, mouse.vs)
        if (d < l) {
  ...
  return (
    <div
      ...
      style={{ backgroundColor, position: 'absolute', top: 0, left: 0, margin: 0, padding: 0, width: '100%', height: '100%', overflow: 'hidden' } as React.CSSProperties}
    >
