...
export function PartnersSection() {
  ...
  return (
    <section id="partners" ref={ref} className="relative py-24 lg:py-36 overflow-hidden" style={{ background: 'linear-gradient(180deg, #08090E 0%, #0A0F1A 50%, #08090E 100%)' }}>
      <div className="container relative z-10">
        <div className="mb-16 transition-all duration-700" style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(20px)' }}>
          <div className="flex items-center gap-3 mb-6"><div className="section-line" /><span className="font-body text-xs uppercase tracking-widest" style={{ color: '#0EA5E9', letterSpacing: '0.2em' }}>Partners</span></div>
        ...
        <div className="mb-16 transition-all duration-700" style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(20px)', transitionDelay: '200ms' }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {titlePartners.map((partner, i) => (
              <a key={i} href={partner.url} target="_blank" rel="noopener noreferrer" className="group p-8 rounded-sm card-hover flex flex-col items-center text-center" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(245, 158, 11, 0.15)', textDecoration: 'none' }}>
        ...
        <div className="transition-all duration-700" style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(20px)', transitionDelay: '400ms' }}>
          ...
          <div className="flex flex-wrap gap-3">
            {officialPartners.map((partner, i) => (
              <a key={i} href={partner.url} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-3 px-5 py-3 rounded-sm transition-all duration-300" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', textDecoration: 'none' }}>
        ...
        <div className="mt-16 p-8 rounded-sm transition-all duration-700" style={{ background: 'rgba(14, 165, 233, 0.04)', border: '1px solid rgba(14, 165, 233, 0.15)', opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(20px)', transitionDelay: '600ms' }}>
          <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
            ...
            <a href="mailto:contact@mathisghio.com" className="flex-shrink-0 px-8 py-3 rounded-sm font-heading font-bold text-sm uppercase tracking-wider text-white transition-all duration-300" style={{ background: 'linear-gradient(135deg, #0EA5E9, #0284C7)', boxShadow: '0 0 20px rgba(14, 165, 233, 0.3)', letterSpacing: '0.15em', textDecoration: 'none' }}>
              Become a Partner
            </a>
          </div>
        </div>
