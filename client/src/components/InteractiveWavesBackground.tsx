...
export function InteractiveWavesBackground({
  ...
  const drawLines = () => {
    linesRef.current.forEach((points, lIndex) => {
      ...
      for (let i = 1; i < points.length; i++) { const current = moved(points[i]); d += `L ${current.x} ${current.y}` }
  ...
  return (
    <div
      ...
      style={{ backgroundColor, position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', overflow: 'hidden', '--x': '-0.5rem', '--y': '50%' } as React.CSSProperties}
    >
