...
const PODIUM_IMG = 'https://d2xsxph8kpxj0f.cloudfront.net/310519663455151996/gejRxzH8i7adBy6yjBjf3b/podium-main_ca0aec18.JPG'
...
export function GoatSection() {
  ...
  return (
    <section ref={ref} className="relative py-24 lg:py-40 overflow-hidden" style={{ background: '#08090E' }}>
      <div className="absolute inset-0 z-0" style={{ backgroundImage: `url(${PODIUM_IMG})`, backgroundSize: 'cover', backgroundPosition: 'center 20%', opacity: 0.35, filter: 'saturate(0.6)' }} />
      ...
      <div className="container relative z-10 text-center">
        <div className="transition-all duration-700" style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(20px)' }}>
          <span className="inline-block font-body text-xs uppercase tracking-widest mb-8 px-4 py-2 rounded-sm" style={{ color: '#F59E0B', background: 'rgba(245, 158, 11, 0.08)', border: '1px solid rgba(245, 158, 11, 0.2)', letterSpacing: '0.25em' }}>The Greatest of All Time</span>
        </div>
        ...
        <div className="mt-16 grid grid-cols-2 lg:grid-cols-4 gap-6 max-w-4xl mx-auto transition-all duration-700" style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(20px)', transitionDelay: '450ms' }}>
          {[{ num: '5', label: 'World Championships', suffix: '×' }, { num: '4', label: 'European Championships', suffix: '×' }, { num: '10', label: 'World Cup Victories', suffix: '+' }, { num: '41.40', label: 'Knots Speed Record', suffix: '' }].map((s, i) => (
            <div key={i} className="text-center">
              <div className="font-display" style={{ fontSize: 'clamp(48px, 6vw, 80px)', color: i === 0 ? '#F59E0B' : '#0EA5E9', textShadow: i === 0 ? '0 0 40px rgba(245, 158, 11, 0.5)' : '0 0 40px rgba(14, 165, 233, 0.5)', lineHeight: 1 }}>{s.num}{s.suffix}</div>
              <div className="font-body text-xs uppercase tracking-wider mt-2" style={{ color: 'rgba(148, 163, 184, 0.6)', letterSpacing: '0.1em' }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
