      ...
      {(['2d','3d'] as const).map(m=>{
        ...
        return (
          <button key={m} onClick={()=>{if(!active)onToggle()}}
            ...
            {m==='2d'?<Map size={13} style={{color:active?'#38bdf8':'rgba(148,163,184,0.75)'}}/>:<Globe size={13} style={{color:active?'#38bdf8':'rgba(148,163,184,0.75)'}}/>}
            <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:'0.7rem',letterSpacing:'0.1em',textTransform:'uppercase',color:active?'rgba(241,245,249,0.95)':'rgba(148,163,184,0.75)',transition:'color 0.2s'}}>
              {m==='2d'?'Flat':'3D'}
            </span>
          </button>
        )
  ...
  return (
    <div ref={domRef} onMouseEnter={onEnter} onMouseLeave={onLeave} onClick={onClick} style={{
      ...
      <div style={{padding:'9px 11px',borderRight:'1px solid rgba(255,255,255,0.04)',minWidth:'52px',flexShrink:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'1.05rem',color:isActive?color:'rgba(148,163,184,0.75)',letterSpacing:'0.06em',lineHeight:1.1,transition:'color 0.25s'}}>{comp.month}</div>
        <div style={{fontSize:'0.52rem',color:'rgba(148,163,184,0.75)',marginTop:'2px',letterSpacing:'0.1em',fontFamily:"'DM Sans',sans-serif"}}>2026</div>
      </div>
      <div style={{flex:1,padding:'9px 11px',minWidth:0}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:'6px'}}>
          <span style={{color:'rgba(148,163,184,0.75)',fontSize:'0.6rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontFamily:"'DM Sans',sans-serif"}}>{comp.location}</span>
          <span style={{color:isActive?color:'rgba(148,163,184,0.75)',fontSize:'0.58rem',flexShrink:0,fontFamily:"'DM Sans',sans-serif",fontWeight:500,transition:'color 0.25s'}}>{comp.date}</span>
        </div>
      </div>
      ...
      <div className="container relative z-10">
        ...
        <div className="grid lg:grid-cols-[1fr_380px] gap-8 lg:gap-10 items-start">
          ...
          <div ref={globeContainerRef} className="relative rounded-sm overflow-hidden h-[300px] sm:h-[420px] lg:h-[520px]" style={{border:'1px solid rgba(14,165,233,0.12)',background:'#020510',boxShadow:'0 0 80px rgba(14,165,233,0.05),inset 0 0 80px rgba(14,165,233,0.03)'}}>
            ...
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 pointer-events-none" style={{color:'rgba(148,163,184,0.75)',fontSize:'10px',fontFamily:"'DM Sans',sans-serif",textTransform:'uppercase',letterSpacing:'0.15em',whiteSpace:'nowrap'}}>
          ...
          <div className="flex flex-col">
            <motion.div initial={{opacity:0,x:20}} animate={inView?{opacity:1,x:0}:{}} transition={{duration:.4,delay:.1}}
              ...
              <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:'0.65rem',color:'rgba(148,163,184,0.75)',letterSpacing:'0.2em',textTransform:'uppercase'}}>12 events · 7 countries</p>
            ...
            <motion.div initial={{opacity:0}} animate={inView?{opacity:1}:{}} transition={{duration:.4,delay:.75}}
              ...
              {(Object.entries(TL) as [CompType,string][]).map(([type,label])=>(
                <div key={type} className="flex items-center gap-1.5">
                  ...
                  <span style={{color:'rgba(148,163,184,0.75)',fontSize:'0.62rem',fontFamily:"'DM Sans',sans-serif",letterSpacing:'0.06em'}}>{label}</span>
                </div>
              ))}
            </motion.div>
          </div>
