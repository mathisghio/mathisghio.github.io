...
export function AboutSection() {
  ...
  return (
    <section id="about" ref={ref} className="relative py-10 lg:py-36 overflow-hidden" style={{ background: 'linear-gradient(180deg, #08090E 0%, #0A0F1A 50%, #08090E 100%)' }}>
      <div className="absolute inset-0 z-0" style={{ backgroundImage: `url(${HERO_IMG})`, backgroundSize: 'cover', backgroundPosition: '15% center', opacity: 0.25, filter: 'saturate(0.5)' }} />
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-24 items-center relative z-10">
          <div className="relative z-10">
            ...
            <div className="mt-6 lg:mt-10 grid grid-cols-3 gap-4 transition-all duration-700" style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(32px)', transitionDelay: '700ms', transitionTimingFunction: 'cubic-bezier(0.16, 1, 0.3, 1)' }}>
              {[{ value: '5×', suffix: '', label: 'World Champion', gold: true }, { value: '4×', suffix: '', label: 'European Champion', gold: false }, { value: '41.40', suffix: 'kts', label: 'Speed Record', gold: false }].map((stat, i) => (
                <div key={i} className="p-4 rounded-sm text-center card-hover" style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${stat.gold ? 'rgba(245, 158, 11, 0.3)' : 'rgba(14, 165, 233, 0.15)'}` }}>
                  ...
                  <div className="font-body uppercase mt-1 whitespace-nowrap" style={{ color: 'rgba(148, 163, 184, 0.7)', fontSize: 'clamp(7px, 1.8vw, 10px)', letterSpacing: '0.04em' }}>
          ...
          <div className="relative z-10 transition-all" style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateX(0)' : 'translateX(64px)', transitionDelay: '150ms', transitionDuration: '1000ms', transitionTimingFunction: 'cubic-bezier(0.16, 1, 0.3, 1)' }}>
            <div className="relative rounded-sm overflow-hidden" style={{ aspectRatio: '4/3.5', border: '1px solid rgba(14, 165, 233, 0.15)', boxShadow: '0 30px 80px rgba(0,0,0,0.5)' }}>
              <img src={ABOUT_WATER} alt="Mathis Ghio wingfoiling on water" className="w-full h-full object-cover" style={{ filter: 'saturate(1.1) contrast(1.05)', objectPosition: 'center 45%' }} />
